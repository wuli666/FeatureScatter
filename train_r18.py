...
parser.add_argument('--model_dir', default='./checkpiont/r18_cifar10',type=str, help='model path')
...
parser.add_argument('--weight_decay',
                    ...
                    type=float,
                    help='weight decay')
parser.add_argument('--log_step', default=1000, type=int, help='log_step')
...
class_ids = [0,1,2]
dataset_set = generate_dataset(trainloader,class_ids=class_ids,transform=None,num_classes=args.num_classes)
...
if args.dataset == 'cifar10' or args.dataset == 'cifar100' or args.dataset == 'svhn':
    print('---wide resenet-----')
    # basic_net = resnet18_cifar(num_classes=args.num_classes)
    # basic_net = resnet34_cifar(num_classes=args.num_classes)
    basic_net = MLP(num_classes=args.num_classes)
...
def train_fun(epoch, net):
    ...
    iterator = tqdm(trainloader1, ncols=0)
    for batch_idx, (inputs, targets) in enumerate(iterator):
        ...
        if batch_idx % args.log_step == 0:
            ...
            iterator.set_description(
                "[Epoch] %d, [step] %d, [lr] %.4f, [duration] %.2f, [training nat acc] %.2f, [training adv] "
                "acc %.2f, [training adv loss] %.4f " % (epoch, batch_idx, lr, duration, 100 * nat_acc,
